...
export interface ConnectionPluginConfig {
   ...
   *
   * Defaults to requiring that either a `first` or `last` is provided, and that after / before must be
   * paired with `first` or `last`, respectively.
   */
   ...
   * `last` field as needed. Defaults to true, setting this to false will disable this behavior and make
   * the field nullable.
   */
  strictArgs?: boolean
  /**
  ...
  extendEdge?: Record<
    ...
    Omit<FieldOutConfig<any, any>, 'resolve'> & {
      /**
       * Set requireResolver to false if you have already resolved this information during the resolve of
       * the edges in the parent resolve method
       *
       * @default true
       */
...
export declare type NodeValue<TypeName extends string = any, FieldName extends string = any> = ResultValue<
...
export declare type ConnectionFieldConfig<TypeName extends string = any, FieldName extends string = any> = {
  ...
  | {
       ...
       *
       * We will then slice the array in the direction we're iterating, and if there are more than "N" results,
       * we will assume there's a next page. If you set `assumeExactNodeCount: true` in the config, we will
       * assume that a next page exists if the length >= the node count.
       */
      nodes: (
        root: SourceValue<TypeName>,
        args: ArgsValue<TypeName, FieldName>,
  ...
  | {
      /**
       * Implement the full resolve, including `edges` and `pageInfo`. Useful in more complex pagination cases,
       * or if you want to use utilities from other libraries like GraphQL Relay JS, and only use Nexus for the
...
export declare type EdgeTypeLookup<TypeName extends string, FieldName extends string> = FieldTypeName<
...
export declare type ConnectionFieldResolver<
  TypeName extends string,
  FieldName extends string,
