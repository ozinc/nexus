...
export interface PluginConfig {
   ...
   * yet. `builderLens.hasType` will only return true for types the user has defined top level in their
   * app, and any types added by upstream plugins.
   */
   ...
   * middlewares with the (root, args, ctx, info, next) signature, where the `next` is the next middleware
   ...
   * of middlewares with the (root, args, ctx, info, next) signature, where the `next` is the next
   * middleware or resolver to be executed.
   */
  onCreateFieldSubscribe?: (createSubscribeInfo: CreateFieldResolverInfo) => MiddlewareFn | undefined
}
export declare function completeValue<T, R>(
...
export declare function completeValue<T, R, E>(
  valOrPromise: PromiseLike<T> | T,
  onSuccess: (completedVal: T) => R,
  onError: (err: any) => R
): R | UnwrapPromise<R> | PromiseLike<UnwrapPromise<R>> | PromiseLike<UnwrapPromise<R>>
 ...
 * @param resolver
 */
export declare function composeMiddlewareFns<T>(
