...
function normalizeArgWrapping(argVal) {
  ...
  if (isNexusWrappingType(argVal)) {
    ...
    if (isNexusArgDef(namedType)) {
      ...
      return args_1.arg(
        Object.assign(Object.assign({}, config), { type: applyNexusWrapping(config.type, wrapping) })
      )
    }
...
function finalizeWrapping(nonNullDefault, typeWrapping, chainWrapping) {
  let finalChain = []
  const allWrapping = typeWrapping.concat(
    chainWrapping !== null && chainWrapping !== void 0 ? chainWrapping : []
  )
