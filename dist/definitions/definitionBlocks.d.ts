...
export declare type CommonOutputFieldConfig<
...
> = CommonFieldConfig & {
   ...
   * "stringArg" and "intArg". You can also use type modifier helpers
   * "[list](https://nxs.li/docs/api/list)" "[nullable](https://nxs.li/docs/api/nullable)" and
   * "[nonNull](https://nxs.li/docs/api/nonNull)". For details about nonNull/nullable refer to the
   ...
   * some graphql-js based tools like [join-monster](https://github.com/join-monster/join-monster) which
   * rely on looking for special data here.
   *
   * @example
...
export declare type CommonInputFieldConfig<
  TypeName extends string,
  FieldName extends string
> = CommonFieldConfig & {
...
export interface OutputScalarConfig<TypeName extends string, FieldName extends string>
   ...
   * explicitly implement every resolver however. If the [source
   * typing](https://nxs.li/guides/backing-types) includes:
   *
...
export interface NexusOutputFieldConfig<TypeName extends string, FieldName extends string>
   ...
   *
   * Object type fields may be typed as scalars or other output types in your schema, often object types.
   * They may also use type modifiers like list and non-null types.
   *
...
export declare type ScalarOutSpread<TypeName extends string, FieldName extends string> = NeedsResolver<
  TypeName,
  FieldName
> extends true
  ...
  ? [ScalarOutConfig<TypeName, FieldName>]
  : [ScalarOutConfig<TypeName, FieldName>] | []
export declare type ScalarOutConfig<TypeName extends string, FieldName extends string> = NeedsResolver<
  TypeName,
  FieldName
> extends true
  ...
  : OutputScalarConfig<TypeName, FieldName>
export declare type FieldOutConfig<TypeName extends string, FieldName extends string> = NeedsResolver<
  TypeName,
  FieldName
> extends true
...
export declare class OutputDefinitionBlock<TypeName extends string> {
   ...
   *
   * Chain this property to wrap the right-hand-side type (the field type, another list, nonNull, etc.) with
   * a List type.
   *
   * Chains are read backwards, right to left, like function composition. In other words the thing on the
   * left wraps the thing on the right.
   *
   ...
   *
   * In Nexus output types are nullable by default so this is useful to configure a field differently. Note
   * if you find yourself using this most of the time then what you probably what is to change the
   * nonNullDefaults configuration either globally in your makeSchema config or at the type definition
   * level in one of your type configs to be false for outputs.
   *
   * Chains are read backwards, right to left, like function composition. In other words the thing on the
   * left wraps the thing on the right.
   *
   ...
   * unicode) character sequences. It is most often used to represent free-form human-readable text. They
   * are represented in JavaScript using the [string primitive
   ...
   * set](https://spec.graphql.org/June2018/#sec-Selection-Sets). They are in fact most of what any
   * selection set will contain. Fields can be typed as scalars (marking the terminal point of a branch of
   * a selection set) or as other object types in your schema thus allowing you to model relationships
   * between things.
   *
   * @example
   *   objectType({
