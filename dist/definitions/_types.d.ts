...
export declare type NexusFeaturesInput = {
  /**
   * Toggle runtime checks for correct implementation of abstract types. This is a redundant check Nexus
   * makes over the existing static typings it provides.
   *
   ...
   * using multiple strategies](https://nxs.li/guides/abstract-types/using-multiple-strategies) as there
  ...
  abstractTypeStrategies?: {
    /**
     * The Modular abstract type strategy. Every member object of an abstract type (union members or
     * interface implementors) will generally be required to implement isTypeOf method. Nexus will not
     * require it in cases where it detects you have implemented another strategy. For more detail see
     * the guide for the [Modular Abstract Type Strategy](https://nxs.li/guides/abstract-types/modular-strategy).
     */
    isTypeOf?: boolean
    /**
     * The Centralized abstract type strategy. Every abstract type (union or interface) will generally be
     * required to implement its resolveType method. Nexus will not require it in cases where it detects
     * you have implemented another strategy. For more detail see the guide for the [Central Abstract
     * Type Strategy](https://nxs.li/guides/abstract-types/centralized-strategy).
     */
    resolveType?: boolean
    /**
     * The Discriminant Model Field strategy. In this mode the resolvers of fields typed as abstract types
     * will be required to include "__typename" field in the returned data. For more detail see the guide
     * for the [Discriminant Model Field
     * Strategy](https://nxs.li/guides/abstract-types/discriminant-model-field-strategy).
     *
     ...
     * practical at runtime to find out if resolvers will return objects that include the "__typename"
     * field. This trade-off can be acceptable since the runtime checks are a redundant safety measure
     * over the static typing. So as long as you are not ignoring static errors related to Nexus'
     * abstract type type checks then you then you should still have a safe implementation.
     *
     * Furthermore another effect is that statically the other strategies will not appear to be
     * _required_, but instead _optional_, while only this one will appear required. However, upon
     * implementing any of the other strategies, this one will not longer be required. This quirk is
     * explained in the guide section about [using multiple
     * strategies](https://nxs.li/guides/abstract-types/using-multiple-strategies).
     */
    __typename?: boolean
  }
}
